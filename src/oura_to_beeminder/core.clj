...
;; e.g. sleep for the night of the 19th of October (waking up on the 20th of October), will be stored in Beeminder as a datapoint on the 19th.
(defn now-oura-format [date offset]
...
(defn get-oura-data [lookback-days]
  ...
  (client/get "https://api.ouraring.com/v1/sleep"
              {:query-params {"start" (now-oura-format (Date.) lookback-days)
                              ;"end" "2020-05-19"
...
(defn get-all-data [lookback-days]
  (let [bmdr-data (->
        ...
        oura-data #p (get-oura-sleep-data lookback-days)]
    (as-> oura-data d
      (filterv #(should-sync % bmdr-data-mod) d)
      #p (mapv #(update-bmdr-data (get % "summary_date") (get % "score") "autosync") d))))
