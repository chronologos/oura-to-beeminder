...
(deftest should-sync-trivial
  (testing "should sync"
    (let [x (should-sync {} [{}] :daystamp)]
      (is (false?
           x)))))

(deftest should-sync-one
  (testing "should sync"
    (let [x (should-sync {:daystamp "1-1-2000"} [{:daystamp "1-1-2000"}] :daystamp)]
      (is (false?
           x)))))
...
(deftest oura-processed-spec
  (testing "oura spec"
    (let [x (get-oura-sleep-data
             (fn [x] {:body
                      oura-json-string-body})  2)]
      (is (true?
           (s/valid? :oura/sleep-data x))
          (s/explain-str :oura/sleep-data x)))))

(deftest oura-json-spec
  (testing "oura json spec"
    (let [x (parse-sleep-data-json
             (fn [x] {:body
                      oura-json-string-body}))]
      (is (true?
           (s/valid? :oura/sleep-data x))
          (s/explain-str :oura/sleep-data x)))))
...
(deftest bmdr-processed-spec
  (testing "bmdr spec"
    (let [x (get-bmdr
             (fn [x] {:body
                      bmdr-json-string-body}) 2)]
      (is (true?
           (s/valid? :bmdr/data x))
          (s/explain-str :bmdr/data x)))))

;; (deftest bmdr-json-spec
;;   (testing "bmdr json spec"
;;     (let [x (get-bmdr
;;              (fn [x] {:body
;;                       bmdr-json-string-body}) 2)]
;;       (is (true?
